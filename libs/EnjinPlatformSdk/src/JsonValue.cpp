...
[[maybe_unused]]
JsonValue JsonValue::GetField(const std::string& key) const
{
    return _pimpl->GetField(key);
}
...
[[maybe_unused]]
bool JsonValue::TryGetField(const std::string& key, JsonValue& outValue) const
{
    return _pimpl->TryGetField(key, outValue);
}
